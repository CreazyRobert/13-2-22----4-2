...
-(id)getGif
{
    ...
    NSData *imageData=[NSData dataWithContentsOfFile:_gifFile];
    
    return [self initWithGIFData:imageData];

}
-(void)setGifData:(NSData *)send
{
    _gifData = send;
    [self getGifData];
}
-(id)getGifData
{
    return [self initWithGIFData:_gifData];
}
/*****************************************/
...
-(void)initWithURL:(NSString *)string
{
    ...
    //        }
    //    else{
    //        string = [string stringByAppendingString:@".gif"];
    //    }
    NSString *suffix = nil;
    ...
    {
        suffix = [string substringFromIndex:range.location];
        string = [string stringByAppendingString:suffix];
    }
    URL = string;
    //    [aview setImageWithURL:[NSURL URLWithString:url]];
    //    NSLog(@"%@",URL);
    NSLog(@"%@",URL);
    NSData *data = [NSData dataWithContentsOfFile:[[SDImageCache sharedImageCache] cachePathForKey:URL]];
    if(data.length < 4)
    {
        //        NSLog(@"1111111111111111%d",isLoading);
        if(manager)
        {
            [manager cancelAllDownloaders];
        }
        isLoading = YES;
        manager = [SDWebImageManager sharedManager];
        [manager downloadWithURL:[NSURL URLWithString:string] delegate:self];
        
    }
    else
    {
        NSString *filePath = [[SDImageCache sharedImageCache] cachePathForKey:URL];
        [self setGifFile:filePath];
        [[NSNotificationCenter defaultCenter]postNotificationName:@"refreshUIImage" object:nil];
...
-(void)webImageManager:(SDWebImageManager *)imageManager didFinishWithImage:(UIImage *)image forURL:(NSURL *)url
{
    ...
    [self setGifData:[NSData dataWithContentsOfURL:url]];
...
{
    //    NSLog(@"%@",url);
    [TKLoadingView showTkloadingAddedTo:self.window title:@"\n  加 载 失 败  \n\n" activityAnimated:NO duration:2.0];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"refreshUIImage" object:nil];
}
