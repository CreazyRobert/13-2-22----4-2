...
#define MaxImageSize 5000000
#define  networkIndictor [SDNetworkActivityIndicator sharedActivityIndicator]
...
+ (id)downloaderWithURL:(NSURL *)url delegate:(id<SDWebImageDownloaderDelegate>)delegate userInfo:(id)userInfo lowPriority:(BOOL)lowPriority
{
    ...
    if (networkIndictor)
    {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:networkIndictor name:SDWebImageDownloadStartNotification object:nil];
        [[NSNotificationCenter defaultCenter] removeObserver:networkIndictor name:SDWebImageDownloadStopNotification object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:networkIndictor
...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    if([delegate respondsToSelector:@selector(imageDownloader:hasDownloadedPercent:)])
        [delegate performSelector:@selector(imageDownloader:hasDownloadedPercent:) withObject:self withObject:[NSNumber numberWithFloat:(float)[imageData length]/datalength]];
}
