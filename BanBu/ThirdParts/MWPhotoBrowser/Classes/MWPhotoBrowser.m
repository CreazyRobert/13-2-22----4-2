    ...
    if ((self = [super init])) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(handleMWPhotoLoadingDidEndNotification:)
                                                   ...
                                                   object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shareToFB) name:@"shareToFB" object:nil];
    }       
    return self;
}
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"shareToFB" object:nil];
...
- (void)viewDidLoad {
//    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(def:) name:@"def" object:nil];
    ...
    _actionButton = [[UIBarButtonItem alloc] initWithCustomView:nil];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        UIBarButtonItem *sendButton = [[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ic_topbar_more.png"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)] autorelease];
        // Set appearance
    ...
    if (_actionsSheet) {
        ...
        if(![UserDefaults valueForKey:@"myID"]){
            self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                    ...
                                                    otherButtonTitles:NSLocalizedString(@"saveButton", nil),  nil] autorelease];
        }
            ...
            if([langauage isEqual:@"zh-Hans"]){
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"cancelNotice", nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:NSLocalizedString(@"saveButton", nil),NSLocalizedString(@"分享到微信、微博", nil),NSLocalizedString(@"分享到QQ空间、人人网", nil),  nil] autorelease];
            }else{
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"cancelNotice", nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:NSLocalizedString(@"saveButton", nil),@"Twitter",@"Facebook",  nil] autorelease];
            }
        }
...
-(void)shareToFB{
    ...
    if (!displayedNativeDialog) {
        [self performPublishAction:^{
            ...
            [FBRequestConnection startForUploadPhoto:[photo underlyingImage]
...
//    BanBu_FaceBookShare *fbShare = [[BanBu_FaceBookShare alloc]initWithText:nil image:[photo underlyingImage]];
//    BanBu_NavigationController *nav = [[BanBu_NavigationController alloc]initWithRootViewController:fbShare];
//    [self presentModalViewController:fbShare animated:YES];
//    [fbShare release];
//    [nav release];
    
}

-(void)def:(NSNotification *)sender{
    [self shareToFB];
    ...
    [pars setValue:[NSString stringWithFormat:@"%@,%@",[(FBSession *)[sender object] accessToken],@"fb"] forKey:@"bindstring"];
...
-(void)setNavigationBarNextViewController{
//    NSString *path = [[NSBundle mainBundle] pathForResource:@"topbar" ofType:@"png"];
//    UIImage *image = [UIImage imageWithContentsOfFile:path];
//    
//    [self.navigationController.navigationItem.backBarButtonItem setBackgroundImage:[[UIImage imageNamed:@"app_nav_back_bg1.png"] stretchableImageWithLeftCapWidth:15.0 topCapHeight:15.0] forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
//
        ...
        if (buttonIndex != actionSheet.cancelButtonIndex) {
            ...
            if([langauage isEqual:@"zh-Hans"]){
                if (buttonIndex == actionSheet.firstOtherButtonIndex) {
                    ...
                    [_assetLibrary saveImage:[[self photoAtIndex:_currentPageIndex] underlyingImage] toAlbum:NSLocalizedString(@"fristTitle", nil) completionBlock:^(NSURL *assetURL, NSError *error) {
...
//                    [self savePhoto]; return;
                    ...
                    ShareViewController *share = [[ShareViewController alloc] initWithURLString:[NSString stringWithFormat:@"%@",[[self photoAtIndex:_currentPageIndex] underlyingURL]]];
                    NSLog(@"%@",[[self photoAtIndex:_currentPageIndex] underlyingURL]);
                    
                    [self setNavigationBarNextViewController];
...
//                    [self presentModalViewController:share animated:YES];
                ...
                /*
                    ...
                    if(![[UserDefaults valueForKey:@"sinaUser"] length]){
                        ...
                        //NSLog(@"%@",_receiveImage);
                        [_wbEngine sendWeiBoWithText:@"分享一个好东西" image:[photo underlyingImage]];
                        [TKLoadingView showTkloadingAddedTo:self.view title:NSLocalizedString(@"sendingNotice", nil) activityAnimated:YES];
//                        [TKLoadingView showTkloadingAddedTo:self.view title:NSLocalizedString(@"sinaBing", nil) activityAnimated:NO duration:1.0];
                    ...
                    if( ![[UserDefaults valueForKey:@"QUser"] length]){
                        ...
                        UIImageView *sendImageView = [[UIImageView alloc]initWithImage:[photo underlyingImage]] ;
                    ...
                    if(![[UserDefaults valueForKey:@"TUser"]length]){
                        ...
                        dispatch_async(GCDBackgroundThread, ^{
                            @autoreleasepool {
                                ...
                                NSError *returnCode = [aEngine postTweet:@"Share a good thing" withImageData:UIImageJPEGRepresentation([photo underlyingImage], 1.0)];
...
//                                    [av show];
                    ...
                    //                if(![[[NSUserDefaults standardUserDefaults]valueForKey:@"FBUser"]isEqualToString:@""]){
        ...
        if(buttonIndex == 0){
            MyWXOpen.scene = WXSceneSession;
                ...
                [MyWXOpen sendImageContentWithImageData:UIImageJPEGRepresentation([[self photoAtIndex:_currentPageIndex] underlyingImage], 1.0) andThumbData:UIImageJPEGRepresentation([[self photoAtIndex:_currentPageIndex] underlyingImage], 0.5)];
            ...
            if([[UserDefaults valueForKey:@"sinaUser"] length]){
                ...
                //NSLog(@"%@",_receiveImage);
                [_wbEngine sendWeiBoWithText:@"分享个好东西" image: [[self photoAtIndex:_currentPageIndex] underlyingImage]];
                [TKLoadingView showTkloadingAddedTo:self.view title:NSLocalizedString(@"sendingNotice", nil) activityAnimated:YES];
            ...
            if([[UserDefaults valueForKey:@"QUser"] length]){
                ...
                NSString *imagePath = [NSTemporaryDirectory() stringByAppendingFormat:@"releaseImage"];
                UIImage *tempImage = [[self photoAtIndex:_currentPageIndex] underlyingImage];
