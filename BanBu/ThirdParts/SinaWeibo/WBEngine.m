...
- (void)saveAuthorizeDataToKeychain
{
    ...
    /************************加的*************************/
    [UserDefaults setValue:[SFHFKeychainUtils getPasswordForUsername:kWBKeychainAccessToken andServiceName:serviceName error:nil] forKey:kWBAccessToken];
}
...
- (void)readAuthorizeDataFromKeychain
{
    ...
    /************************加的*************************/
    self.accessToken = [UserDefaults objectForKey:kWBAccessToken];
}
...
- (void)deleteAuthorizeDataInKeychain
{
    ...
    /************************加的*************************/
    [UserDefaults removeObjectForKey:kWBAccessToken];
...
- (void)logIn
{
    ...
    {
        [authorize setRedirectURI:@"http://www.halfeet.com"];
    }
...
- (void)getUserInfo
{
    [self loadRequestWithMethodName:@"2/users/show.json"
                     httpMethod:@"GET"
                         params:nil
...
- (void)authorize:(WBAuthorize *)authorize didSucceedWithAccessToken:(NSString *)theAccessToken userID:(NSString *)theUserID expiresIn:(NSInteger)seconds
{
    ...
    //将新浪授权返回的access_token,上传至半步服务器。
    NSString *name =theUserID;
    if(name)
    {
        
        [UserDefaults setValue:name forKey:@"sinaUser"];
    ...
    [pars setValue:[NSString stringWithFormat:@"%@,%@",[UserDefaults valueForKey:kWBAccessToken],[UserDefaults valueForKey:@"sinaUser"]] forKey:@"bindstring"];
